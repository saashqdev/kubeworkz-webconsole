...
wget -d "${headers[@]}" https://kubeworkz:7443/api/v1/kube/user/kubeconfigs -O $DIR/tmp/$TMP_CONFIG_NAME-base64 --no-check-certificate &> /dev/null
# check whether kubeconfig download success
if [ $? -ne 0 ]; then
    wget -d "${headers[@]}" http://kubeworkz:7443/api/v1/kube/user/kubeconfigs -O $DIR/tmp/$TMP_CONFIG_NAME-base64 --no-check-certificate &> /dev/null
    if [ $? -ne 0 ]; then
    exit 1
    fi
fi
